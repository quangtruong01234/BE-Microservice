...
**Recommended order:** SCALE-06 re-runs. SCALE-01..05 + SCALE-07 are
DONE. Remaining: SCALE-06 re-run behind nginx on the target VPS (attributes
...
- [x] **DEP-01 — DONE 2026-07-22** (see CHANGELOG): `npm audit fix` (non-force,
      safe compatible upgrades — 44 packages changed) cleared ALL runtime-exposed
      findings and dropped the report **28 → 10** (2 crit + 12 high → 0). Gone:
      axios, typeorm 0.3.28→0.3.31 (SQLi orderBy/MySQL), ws 8.20.1→8.21.1 +
      engine.io/socket.io-adapter (WS DoS), form-data, multer + @nestjs/platform-
      express 11.1.28, @nestjs/swagger + js-yaml, qs/body-parser, fast-uri,
      brace-expansion, @babel/core, and the crit/high `@xhmikosr/decompress` +
      `piscina` (RCE) + `shell-quote`. Validated green: tsc 0, build 10/10
      services, eslint 0 errors, Jest 176/176.
      **Accepted residual — 10 moderate, dev-build-tool-only, NOT worth fixing:**
      all are `file-type`/`@xhmikosr/*` DoS reachable ONLY via the build chain
      `@nestjs/cli@11.0.21 → @swc/cli@0.6.0 → @xhmikosr/*` — zero production
      runtime exposure (build-time only). `npm audit fix --force` would jam
      `@swc/cli@0.8.1` (breaking) under @nestjs/cli's 0.6.x range and likely
      break `nest build`, so it is deliberately NOT run. (Correction to the
      original triage: `@swc/cli` is a transitive of `@nestjs/cli`, not a dead
      direct dep — the redundant direct devDep entry was removed, but the cluster
...
- GHN address ids at checkout (GHN-ADDR-01, 2026-07-23): `orders.to_district_id` INT NULL + `to_ward_code` VARCHAR(20) NULL persist the exact GHN location captured from the FE checkout dropdowns (optional `toDistrictId`/`toWardCode` on `POST /api/order` + `POST /api/order/shipping-fee`; threaded gateway→orders TCP→`placeOrder`/`placeMultiSellerOrder`/`calculateShippingFee`). `GhnService.buildShippingOrderBody`/`previewShippingFee`/`createShippingOrder` take an optional `GhnResolvedAddress`; when both ids are present the waybill + fee preview use them directly and the free-text master-data resolution below is SKIPPED. `toResolvedAddress`/`toGhnResolvedAddress` guards treat a partial pair (one id missing) as absent → free-text fallback. Migration `nodeA-20260723-001-add-ghn-ids-to-orders` (idempotent, INFORMATION_SCHEMA-guarded; orders runs `synchronize:true` so dev auto-adds the columns). **Applied to prod Aiven Node A 2026-07-30, BEFORE the code deploy** — prod forces `synchronize:false` so the columns never auto-appear there, and because they are extra nullable columns they are inert to the old deployed code (zero downtime, no restart needed). Post-migration smoke on prod: 4/4 order read paths 200 under the old code.
- GHN address resolution (`apps/orders/src/ghn/ghn.service.ts`, free-text fallback when the checkout did NOT send `toDistrictId`/`toWardCode`): the free-text ward/district/province are resolved to GHN numeric `to_district_id` + string `to_ward_code` at waybill-create time via master-data (`GET /master-data/province|district|ward`, 24h TTL cache) using `normalizeAddressPart` (NFD diacritic strip, đ→d, drops VN admin prefixes, alnum-only) + `rankMasterDataMatches` (exact-normalized first, then containment by closest length). The resolver walks ALL ranked province→district→ward candidates and falls through stub entries — required because the GHN dev sandbox has polluted duplicate provinces (e.g. "Hà Nội 02" ProvinceID 2002 with 0 districts shares the "hanoi" alias with the real Hà Nội 201). Unresolvable address → `BadRequestException` (400).
...
- Catalog bootstrap on a fresh environment (SEED-01, 2026-07-29): an empty
  catalog makes `scripts/load/baseline.mjs` abort with `No prod_ product found`.
  The prod baseline SQL seeds NO categories, and product create rejects
  non-`active` categories while shop-submitted categories stay `pending` — so a
  shop account alone cannot bootstrap. Run `node scripts/seed/seed-products.mjs
--count 8` (credentials in gitignored `api/.env.seed`, template
  `.env.seed.example`; needs a shop AND an admin account the first time) to
  create+approve 5 categories and N image-less products with `stockQuantity`
