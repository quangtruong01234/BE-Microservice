...
export class ProductService {
  ...
  constructor(
    @Inject(NAME_SERVICE_TCP.PRODUCT_SERVICE) private readonly productClient: ClientProxy,
    @Inject(NAME_SERVICE_TCP.INVENTORY_SERVICE) private readonly inventoryClient: ClientProxy,
  ...
  async createProduct(dto: CreateProductDto) {
    try {
      ...
      const response = await firstValueFrom(
        this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_CREATE, dto).pipe(
      ...
      MicroserviceErrorHandler.handleError(error, 'create product', 'Product Service');
    }
  }
  ...
  async getAllProducts(query: GetProductsQueryDto) {
    try {
      ...
      const response = await firstValueFrom(
        this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_FIND_ALL, query).pipe(
          timeout(10000),
      ...
      // Debug logging
      this.logger.debug(`Raw response from product service:`, JSON.stringify(response, null, 2));
      
      // Extract products from response structure
      const products = response?.items || response?.data || response || [];
      this.logger.debug(`Extracted products type: ${typeof products}, isArray: ${Array.isArray(products)}, length: ${products?.length || 0}`);
      
      // Enrich products with user information
      const productsArray = Array.isArray(products) ? products : [];
      const enrichedProducts = await this.enrichProductsWithUserInfo(productsArray);
      
      this.logger.log(`Found ${enrichedProducts?.length || 0} products with user info`);
      return enrichedProducts;
    } catch (error) {
      MicroserviceErrorHandler.handleError(error, 'fetch products', 'Product Service');
    }
  }
  ...
  async getProductById(id: number) {
    try {
      ...
      const response = await firstValueFrom(
        this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_FIND_BY_ID, id).pipe(
          timeout(10000),
          catchError((error) => throwError(() => error))
        )
      );
      ...
      this.logger.debug(`Product ${id} response:`, product ? 'found' : 'not found');
      
      // Enrich with user information
      return await this.enrichProductWithUserInfo(product);
    } catch (error) {
      MicroserviceErrorHandler.handleError(error, `fetch product by ID: ${id}`, 'Product Service');
    }
  }
  ...
  async getProductBySku(sku: string) {
    try {
      return await firstValueFrom(
        this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_FIND_BY_SKU, sku).pipe(
          timeout(10000),
          catchError((error) => throwError(() => error))
        )
      );
    } catch (error) {
      MicroserviceErrorHandler.handleError(error, `fetch product by SKU: ${sku}`, 'Product Service');
    }
  }
  ...
  async updateProduct(id: number, dto: UpdateProductDto) {
    try {
      return await firstValueFrom(
        this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_UPDATE, { id, updateProductDto: dto }).pipe(
          timeout(10000),
          catchError((error) => throwError(() => error))
        )
      );
    } catch (error) {
      MicroserviceErrorHandler.handleError(error, `update product ID: ${id}`, 'Product Service');
    }
  }
  ...
  async getProductsByCategory(categoryId: number, query: GetProductsQueryDto) {
    return await firstValueFrom(
      this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_FIND_BY_CATEGORY, { categoryId, query })
    );
  }

  async getProductsByBrand(brandId: number, query: GetProductsQueryDto) {
    return await firstValueFrom(
      this.productClient.send(PRODUCT_MESSAGE_PATTERNS.PRODUCT_FIND_BY_BRAND, { brandId, query })
    );
  }
  ...
  async getProductWithInventoryById(productId: number): Promise<ProductWithInventory | null> {
    try {
      ...
      const [product, inventory] = await Promise.all([
        this.getProductById(productId).catch(err => {
          this.logger.warn(`Product service error for ID ${productId}: ${err.message}`);
          return null;
        }),
        firstValueFrom(
          this.inventoryClient.send(INVENTORY_MESSAGE_PATTERNS.INVENTORY_FIND_BY_PRODUCT_ID, productId)
        ).catch(err => {
          this.logger.warn(`Inventory service error for product ID ${productId}: ${err.message}`);
          return null;
        })
      ]);
      ...
      this.logger.debug(`Successfully aggregated data for product ID: ${productId}`);
      return result;
    } catch (error) {
      this.logger.error(`Aggregation failed for product ID ${productId}:`, error);
      throw error;
    }
  }
  ...
  private getStockStatus(inventory: any): string {
    ...
    if (inventory.availableStock > inventory.minimumStock * 3) return 'IN_STOCK';
    return 'NORMAL_STOCK';
  }

  async getProductsWithInventory(productIds: number[]): Promise<ProductWithInventory[]> {
    try {
      ...
      const [products, inventoryItems] = await Promise.all([
        ...
        firstValueFrom(this.inventoryClient.send(INVENTORY_MESSAGE_PATTERNS.INVENTORY_GET_BY_PRODUCT_IDS, productIds))
      ]);
      ...
      const enrichedProducts = await this.enrichProductsWithUserInfo(validProducts);
  ...
  async getAllProductsWithInventory(query: GetProductsQueryDto): Promise<ProductWithInventory[]> {
    try {
      ...
      const inventoryItems = await firstValueFrom(
        this.inventoryClient.send(INVENTORY_MESSAGE_PATTERNS.INVENTORY_GET_BY_PRODUCT_IDS, productIds)
      );
  ...
  async checkProductStock(productId: number, quantity: number) {
    try {
      return await firstValueFrom(
        this.inventoryClient.send(INVENTORY_MESSAGE_PATTERNS.INVENTORY_CHECK_STOCK, { productId, quantity })
      );
    } catch (error) {
  ...
  private async enrichProductWithUserInfo(product: any) {
    if (!product || typeof product !== 'object' || !product.userId) {
      this.logger.debug(`Skipping user enrichment for product: ${product?.id || 'unknown'}`);
      return product;
    }
    ...
    try {
      ...
      if (isNaN(userId)) {
        this.logger.warn(`Invalid userId for product ${product.id}: ${product.userId}`);
        return product;
      }
      ...
      this.logger.debug(`User enrichment failed for product ${product.id}, userId: ${product.userId}`, error.message);
  ...
  private async enrichProductsWithUserInfo(products: any[]) {
    ...
    try {
      ...
      this.logger.debug(`Fetching user info for userIds: ${JSON.stringify(userIds)}`);
      ...
      return products.map(product => ({
        ...product,
        user: product.userId ? userMap.get(parseInt(product.userId)) || null : null
      }));
