...
/**
 * Global RPC Exception Filter for Rewards Service
...
export class AllRpcExceptionFilter extends BaseRpcExceptionFilter {
  ...
  private handleDatabaseError(exception: any): RpcException {
    ...
    if (message.includes('duplicate key value violates') || exception.code === '23505') {
      ...
      if (message.includes('user_id')) {
        userMessage = 'Reward for this user already exists';
      } else if (message.includes('order_id')) {
        userMessage = 'Reward for this order already exists';
      }
    ...
    if (message.includes('null value in column') || exception.code === '23502') {
      return new RpcException({
        ...
        message: 'Required reward field is missing',
        error: 'Bad Request',
        timestamp: new Date().toISOString(),
      });
    }
    ...
    if (message.includes('foreign key constraint') || exception.code === '23503') {
      return new RpcException({
        statusCode: HttpStatus.BAD_REQUEST,
        message: 'Referenced user or order does not exist',
        error: 'Bad Request',
        timestamp: new Date().toISOString(),
      });
    }
    ...
    if (message.includes('insufficient points') || message.includes('not enough points')) {
      return new RpcException({
    ...
    return new RpcException({
      statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
      message: 'Rewards operation failed',
      error: 'Internal Server Error',
      timestamp: new Date().toISOString(),
    });
  }
  ...
  private handleValidationError(exception: any): RpcException {
    let message = 'Rewards validation failed';
