...
- Cross-check `context/database.md` for indexes/relations and `context/api.md` for current response shape **before** proposing any change that touches them.
...
- Map each WHERE / ORDER BY / LIKE column on hot paths to `context/database.md`; flag columns with no index.
