...
function req(
  ...
  urlKind,
  path,
  { body, rawBody, query, test, prerequest, desc } = {},
) {
  ...
  if (query && query.length)
    url.query = query.map((q) => ({ key: q.key, value: String(q.value) }));
...
const buyerFlow = folder(
  ...
  [
    ...
    req("Preview shipping fee", "POST", "api", "order/shipping-fee", {
      body: {
        shippingAddress:
          "E2E Buyer|0900000000|12 Nguyen Hue|Phường Bến Nghé|Quận 1|Hồ Chí Minh",
        items: [
          { productName: "{{e2eProductName}}", quantity: 1, weight: 500 },
        ],
      },
      test: [
        ...UNWRAP,
    ...
    req(
      ...
      "GET",
      "api",
      "order/user/{{e2eBuyerId}}/status-counts",
      {
        test: [
          ...UNWRAP,
...
const adminFlow = folder(
  ...
  [
    ...
    req(
      ...
      "PATCH",
      "api",
      "order/admin/vouchers/{{e2eVoucherId}}/deactivate",
      {
        test: [
          ...UNWRAP,
...
const shippingFlow = folder(
  ...
  [
    ...
    req(
      ...
      "GET",
      "api",
      "order/admin/ghn/orders/{{e2eGhnOrderId}}",
      {
        prerequest: [
    ...
    req(
      "GHN shipping history",
      "GET",
      "api",
      "order/admin/ghn/orders/{{e2eGhnOrderId}}/history",
      {
        prerequest: [
          'if (!pm.collectionVariables.get("e2eGhnOrderId")) { console.log("skip: no GHN order"); pm.execution.skipRequest && pm.execution.skipRequest(); }',
        ],
    ...
    req(
      ...
      "POST",
      "api",
      "order/admin/ghn/orders/{{e2eGhnOrderId}}/sync",
      {
...
const lifecycleFlow = folder(
  ...
  [
    ...
    req(
      "Seller: ready-to-ship (creates GHN waybill)",
      "PATCH",
      "api",
      "order/{{e2eLcOrderId}}/ready-to-ship",
      {
    ...
    req(
      "Shipping: demo-status delivering",
      "POST",
      "api",
      "order/admin/ghn/orders/{{e2eLcOrderId}}/demo-status",
      {
    ...
    req(
      "Shipping: demo-status delivered",
      "POST",
      "api",
      "order/admin/ghn/orders/{{e2eLcOrderId}}/demo-status",
      {
    ...
    req(
      ...
      "GET",
      "api",
      "order/admin/ghn/orders/{{e2eLcOrderId}}",
      {
        test: [
          ...UNWRAP,
...
console.log(
  `Wrote ${out} — ${collection.item.length} flow folders, ${count} requests.`,
);
